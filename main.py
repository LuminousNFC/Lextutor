...
async def analyser_contenu_gpt4(question: str, websocket: WebSocket) -> Dict[str, Any]:
    messages = [
        ...
        async with asyncio.timeout(30):  # Timeout de 30 secondes
            ...
            async for chunk in response:
                if chunk.choices[0].delta.content is not None:
                    ...
                    await websocket.send_json({"type": "assistantResponseChunk", "data": chunk.choices[0].delta.content})
        
        logger.info("Réponse reçue de OpenAI")
        return {"assistantResponse": full_response}
    except asyncio.TimeoutError:
        logger.error("Timeout lors de la requête à OpenAI")
        raise
...
async def extract_fedlex_article(law_code: str, article_number: str, websocket: WebSocket) -> Dict[str, Union[bool, List[Dict[str, str]]]]:
    cache_key = f"{law_code}-{article_number}"
    if cache_key in article_cache:
        await websocket.send_json({"type": "articleExtracted", "data": article_cache[cache_key]})
        return article_cache[cache_key]

    try:
        normalized_law_code = normalize_law_code(law_code)
        if normalized_law_code is None:
            await websocket.send_json({"type": "error", "data": f"Code de loi non reconnu: {law_code}"})
        ...
        for number in article_numbers:
            ...
            if result.get("success"):
                ...
                article_cache[f"{law_code}-{number_str}"] = result
                await websocket.send_json({"type": "articleExtracted", "data": result})
            else:
                error_message = {"error": result.get('error', 'Erreur inconnue')}
                articles_extracted.append(error_message)
        ...
        logger.error(traceback.format_exc())
        await websocket.send_json({"type": "error", "data": f"Erreur lors de l'extraction: {str(e)}"})
...
async def extract_jurisprudence(keywords: List[str], websocket: WebSocket) -> List[Dict[str, Any]]:
    tasks = [fetch_jurisprudence(keyword, websocket) for keyword in keywords]
    ...
    for result in results:
        if isinstance(result, Exception):
            ...
            for jurisprudence in result:
                await websocket.send_json({"type": "jurisprudenceExtracted", "data": jurisprudence})
    return jurisprudence_results

async def fetch_jurisprudence(keyword: str, websocket: WebSocket) -> List[Dict[str, Any]]:
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            error_message = f"Erreur lors de l'extraction de la jurisprudence pour {keyword}: {result.stderr}"
            logger.error(error_message)
            await websocket.send_json({"type": "error", "data": error_message})
            return []
    except Exception as e:
        error_message = f"Erreur inattendue lors de l'extraction de la jurisprudence pour {keyword}: {str(e)}"
        logger.error(error_message)
        logger.error(traceback.format_exc())
        await websocket.send_json({"type": "error", "data": error_message})
        return []

async def process_question(question: str, keywords: List[str], websocket: WebSocket) -> Dict[str, Any]:
    try:
        logger.info(f"Traitement de la question : {question}")

        analysis_result = await analyser_contenu_gpt4(question, websocket)

        if "error" in analysis_result:
            await websocket.send_json({"type": "error", "data": analysis_result["error"]})
            return {"error": analysis_result["error"]}

        if not analysis_result or "assistantResponse" not in analysis_result:
            error_message = "Erreur lors de l'analyse GPT-4"
            await websocket.send_json({"type": "error", "data": error_message})
        ...
        # Send the parsed result only once
        ...
        articles_tasks = [extract_fedlex_article(law_code, article_number, websocket) for law_code, article_number in articles_to_extract]
        ...
        for article_result in articles:
            if isinstance(article_result, Exception):
                logger.error(f"Erreur lors de l'extraction d'un article : {str(article_result)}")
                await websocket.send_json({"type": "error", "data": f"Erreur lors de l'extraction d'un article : {str(article_result)}"})
            elif article_result.get('success') and 'articles' in article_result:
                ...
                logger.error(f"Résultat inattendu lors de l'extraction d'un article : {article_result}")
                await websocket.send_json({"type": "error", "data": f"Résultat inattendu lors de l'extraction d'un article"})

        jurisprudence = await extract_jurisprudence(keywords, websocket)

        # Send a completion message without including the assistant's response again
        ...
        logger.error(traceback.format_exc())
        await websocket.send_json({"type": "error", "data": f"Erreur interne du serveur: {str(e)}"})
...
async def process_request(request: Request) -> JSONResponse:
    try:
        ...
        result = await process_question(question, keywords, None)
...
async def fetch_article(request: Request) -> JSONResponse:
    try:
        ...
        article_result = await extract_fedlex_article(law_code, article_number, None)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        async for data in websocket.iter_json():
            ...
            try:
                await process_question(question, keywords, websocket)
            except Exception as e:
