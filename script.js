const CONFIG = {
    ...
    FETCH_TIMEOUT: 60000, // Timeout ajusté à 60 secondes
};
...
const jurisprudenceCache = new Map();
...
function decodeAndNormalizeText(text) {
    ...
    // Gérer les caractères spéciaux courants en français
    const specialChars = {
...
function displayRealTimeData(data) {
    ...
    if (data.jurisprudence && Array.isArray(data.jurisprudence)) {
        displayJurisprudence(data.jurisprudence);
    }
}
...
function formatArticleContent(article) {
    ...
    const linkText = fedlexLink
        ? `<a href="${fedlexLink}" target="_blank">Voir sur Fedlex</a>`
        : "Lien non disponible";
    return `
...
class LegalAnalyzer {
    ...
    async handleWebSocketMessage(data) {
        ...
        switch (data.type) {
            case "assistantResponse":
                this.displayAccumulatedAssistantResponse();
                appendMessageAndSave("assistant", data.data);
                break;
            case "assistantResponseChunk":
                this.handleAssistantResponseChunk(data.data);
                break;
            case "analysis":
                // Traiter l'analyse si nécessaire
                break;
            case "article":
                if (data.data) {
    ...
    handleAssistantResponseChunk(chunk) {
        this.accumulatedAssistantResponse += chunk;
        this.updateAssistantResponseUI(this.accumulatedAssistantResponse);
    }

    updateAssistantResponseUI(accumulatedResponse) {
        if (!this.currentMessageElement) {
            this.currentMessageElement = document.createElement("div");
            this.currentMessageElement.className = "message assistant-message";
            elements.messagesContainer.appendChild(this.currentMessageElement);
        }
        this.currentMessageElement.innerHTML = formatAssistantResponse(accumulatedResponse);
        this.currentMessageElement.scrollIntoView({ behavior: 'smooth', block: 'end' });
    }

    displayAccumulatedAssistantResponse() {
        if (this.accumulatedAssistantResponse) {
            appendMessageAndSave("assistant", this.accumulatedAssistantResponse);
            this.accumulatedAssistantResponse = "";
            this.currentMessageElement = null;
        }
    }
    ...
    async processQuestion(question, keywords) {
        ...
        const requestData = {
            question,
            keywords: keywords.filter(Boolean), // Filtrer pour enlever les champs vides
        };
    ...
    async sendQuestionViaWebSocket(data) {
        ...
        if (this.socket.readyState === WebSocket.OPEN) {
            ...
            console.log("Question envoyée avec succès");
        } else {
